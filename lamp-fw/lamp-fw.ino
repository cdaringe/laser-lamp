...
byte minDistance=5;  // cm (min distance === min brightness)
byte maxDistance=100; // cm (max distance === max brightness)
...
void loop() {
  ...
  if (distance < minDistance) {
    ...
    currentBrightness = map(distance, minDistance, maxDistance, 1, 255);
